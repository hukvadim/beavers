...
<div class="admin-header">
	<div class="admin-header__title-hold">
		<h2 class="admin-header__title">Article edit</h2>
	</div>
</div>
...
<form class="admin-form-edit form-style js-send-form" enctype="multipart/form-data">
	...
	<input type="hidden" name="form-type" value="admin-article-edit">
	...
	<div class="box-input d-flex align-items-center">
		...
		<div class="box-input-img-select">
			<label class="select-file select-file--btn has-preview" id="set-image">
				...
				<img src="<?=viewImg($recordData['img'], 'article').addTmpView()?>" class="select-file__img-preview" id="img-preview">
			</label>
		</div>
	</div>
		...
		<select name="category" class="form-control form-control-lg form-select" id="input-category" required style="max-width: 400px;">
			...
			<?php if (arrExist($rivers)): ?>
				<?php foreach ($rivers as $value): ?>
					<option value="<?=$value['id']?>" <?=isActive($value['id'], $recordData['cat_id'], 'selected')?>><?=viewStr($value['title'])?></option>
				<?php endforeach ?>
			<?php endif ?>
		</select>
	</div>
	...
	<div class="box-input d-flex align-items-center">
		...
		<input type="text" value="<?=viewValue($recordData['title'])?>" id="input-title" class="form-control form-control-lg" name="title" placeholder="What will your article be called?" maxlength="100" minlength="4" required>
	</div>
	<div class="box-input d-flex">
		<label for="input-text-small" class="box-input__label fw-bold"><span class="required-label">Description</span></label>
		<textarea name="text-small" id="input-text-small" class="form-control form-control-lg" minlength="10" maxlength="200" placeholder="Write a free-form small text max 200 characters" required><?=viewStr($recordData['text_sm'])?></textarea>
	</div>
	<div class="box-input d-flex">
		<label for="input-article" class="box-input__label fw-bold"><span class="required-label">Article</span></label>
		<textarea name="article" id="input-article" class="form-control form-control-lg" minlength="10" placeholder="Write a free-form article" required style="--min-h: 250px"><?=viewStr($recordData['text'])?></textarea>
	</div>

	<div class="box-submit d-flex gap-4 mt-4 label-offset">
		<button class="btn btn-submit btn-primary btn-lg">Edit article</button>
